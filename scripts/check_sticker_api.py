...
def main() -> int:
    ...
    if items:
        item = items[0]
        check("项字段齐全", set(item) == {"id", "name", "label", "category", "has_thumb", "thumbBytes"}, str(item))
        check("has_thumb 为真", item.get("has_thumb") is True)
        # thumbBytes 是「这张图传到手表上要多少字节」，0 说明没算出来 —— 手表拿它
        # 估算流量，报 0 会让它以为免费，把所有图都排进加载队列。
        check("thumbBytes 是真实字节数", (item.get("thumbBytes") or 0) > 0, str(item.get("thumbBytes")))
